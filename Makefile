...
RELEASE = 1
SPEC = rpmbuild/SPECS/puppet-keepalived.spec
...
docs: puppet-keepalived-documentation.pdf

puppet-keepalived-documentation.pdf: DOCUMENTATION.md
	pandoc DOCUMENTATION.md -o 'puppet-keepalived-documentation.pdf'
...
$(RPM): $(SPEC) $(SOURCE)
	...
	mv rpmbuild/RPMS/noarch/puppet-keepalived-$(VERSION)-$(RELEASE).*.rpm $(RPM)
...
$(SRPM): $(SPEC) $(SOURCE)
	...
	#mv rpmbuild/SRPMS/puppet-keepalived-$(VERSION)-$(RELEASE).*.src.rpm $(SRPM)

#
#	spec
#
$(SPEC): rpmbuild/ puppet-keepalived.spec.in
	@echo Running templater...
	#cat puppet-keepalived.spec.in > $(SPEC)
	sed -e s/__VERSION__/$(VERSION)/ -e s/__RELEASE__/$(RELEASE)/ < puppet-keepalived.spec.in > $(SPEC)
...
$(SOURCE): rpmbuild/
	...
	git archive --prefix=puppet-keepalived-$(VERSION)/ -o $(SOURCE) $(VERSION) 2> /dev/null || (echo 'Warning: $(VERSION) does not exist.' && git archive --prefix=puppet-keepalived-$(VERSION)/ -o $(SOURCE) HEAD)
